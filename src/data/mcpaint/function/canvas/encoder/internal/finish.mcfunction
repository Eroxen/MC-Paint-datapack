...
data modify storage mcpaint:calc internal.canvas.encoder.rows set value {}
...
execute store result storage mcpaint:calc internal.canvas.encoder.macro.i int 1 run scoreboard players get #canvas.i mcpaint.calc
function ~/glue_rows with storage mcpaint:calc internal.canvas.encoder.macro
...
function ~/glue_rows:
  here = ~/
  raw f"$function {here}/$(i) with storage mcpaint:calc internal.canvas.encoder.rows"

  for i in range(64):
    arguments = ""
    for j in range(i + 1):
      arguments += f"$({j})"
    command = f'$data modify storage mcpaint:calc internal.canvas.encoder.output.clipboard_text set value "{arguments}"'
    function f"{here}/{i}":
      raw command