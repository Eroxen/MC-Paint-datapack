...
execute unless data storage mcpaint:calc internal.paint_bucket.parse{char:"#"} on target run return run tellraw @s {"translate":"mcpaint.error.invalid_html_colour","color":"red","with":[{"storage":"mcpaint:calc","interpret":true,"nbt":"internal.paint_bucket.parse.str"}]}
execute store result score #temp mcpaint.calc run data get storage mcpaint:calc internal.paint_bucket.parse.str
execute unless score #temp mcpaint.calc matches 7 on target run return run tellraw @s {"translate":"mcpaint.error.invalid_html_colour","color":"red","with":[{"storage":"mcpaint:calc","interpret":true,"nbt":"internal.paint_bucket.parse.str"}]}
