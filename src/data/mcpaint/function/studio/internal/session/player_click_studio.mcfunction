...
execute if items entity @s weapon.mainhand *[custom_data~{mcpaint:{projector_tape:1b}}] run return run execute at @e[type=marker,tag=mcpaint.studio.marker,tag=mcpaint.studio.in_session,predicate=mcpaint:same_session,limit=1] run function mcpaint:studio/internal/session/use_item/projector_tape
