...
execute if score #studio.session.owned_by_another mcpaint.calc matches 1 run return run tellraw @s {"translate":"mcpaint.error.studio_owned_by_another","color":"red","with":[{"storage":"mcpaint:calc","interpret":true,"nbt":"api.database.author_name"}]}
