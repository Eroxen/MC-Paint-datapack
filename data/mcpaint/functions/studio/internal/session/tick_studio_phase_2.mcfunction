...
execute as @e[type=text_display,tag=mcpaint.studio.ui_element.parent,distance=..0.1,predicate=mcpaint:ui_overlap,limit=1] run function mcpaint:studio/internal/ui/hover_tick
...
execute if score #studio.session.click mcpaint.calc matches 1 as @e[type=text_display,tag=mcpaint.studio.ui_element.parent,distance=..0.1,predicate=mcpaint:ui_overlap,limit=1] on passengers if entity @s[type=marker] run function mcpaint:studio/internal/ui/click_on_marker with entity @s data.definition
