...
data modify storage mcpaint:calc array_op.old set from storage mcpaint:calc canvas.resolved_rows
data modify storage mcpaint:calc array_op.updated set from storage mcpaint:calc updated_rows
scoreboard players set current_row mcpaint.calc 0
execute if data storage mcpaint:calc array_op.array[0] if data storage mcpaint:calc array_op.updated[0] as @e[type=minecraft:item_display,tag=mcpaint.resolve_text,distance=..0.1,limit=1] run function mcpaint:canvas/colour_array_to_resolved_rows_updated_loop
data modify storage mcpaint:calc array_op.resolved_rows append from storage mcpaint:calc array_op.old[]
